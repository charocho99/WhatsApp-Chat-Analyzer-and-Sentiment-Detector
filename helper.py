...
def sentimentAnalysis(selected_user, df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
    ...
    df["neutral"] = [sentiments.polarity_scores(
        i)["neu"] for i in df["message"]]
    temp_df = df.drop(columns=['date', 'year', 'month',
                      'day', 'hour', 'minute', 'only_date', 'month_num', 'day_name', 'period'], axis=1)
    return temp_df
